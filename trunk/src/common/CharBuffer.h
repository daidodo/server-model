...
class CCharBuffer
{
    typedef CCharBuffer<Char> __Myt;
    typedef Char __Char;
public:
    ...
    int compare(size_type offset, size_type count, const_pointer buf, size_type newCount) const{
        ...
        if(c){
            assert(buf_ && buf);
            ret = traits_type::compare(buf_, buf, c);
        }
        if(!ret)
...
template<typename Char>
bool operator ==(const CCharBuffer<Char> & left, const CCharBuffer<Char> & right)
{
    return (left.size() == right.size() && 0 == left.compare(right));
}

template<typename Char>
bool operator ==(const Char * left, const CCharBuffer<Char> & right)
{
    return (0 == right.compare(left));
}

template<typename Char>
bool operator ==(const CCharBuffer<Char> & left, const Char * right)
{
...
template<typename Char>
void swap(const CCharBuffer<Char> & left, const CCharBuffer<Char> & right)
{
    left.swap(right);
}
